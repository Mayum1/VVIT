...
from PyQt5.QtWidgets import (QApplication, QWidget, QTabWidget, QAbstractScrollArea, QVBoxLayout, QHBoxLayout,
                             QTableWidget, QGroupBox, QTableWidgetItem, QPushButton, QMessageBox, QButtonGroup,
                             QAbstractButton)
...
class MainWindow(QWidget):
    ...
    def _create_table(self, table_gbox):
        ...
        self.table.setHorizontalHeaderLabels(["Subject", "Room_numb", "Time", "Top/Bottom"])
    ...
    def _update_table(self, table_gbox):
        self.cursor.execute(
            "SELECT * FROM timetable WHERE day='{}' and (week='{}' OR week=2)".format(table_gbox, type_of_week()))
    ...
    def _del_row_table(self, button):
        ...
        try:
            self.cursor.execute("DELETE FROM timetable.timetable WHERE day='{}' and subject='{}'"
                                " and room_numb='{}' and start_time='{}' and week='{}'".format(com[1], row[0], row[1],
                                                                                               row[2], row[3]))
            self.conn.commit()
